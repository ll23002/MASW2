...
def Sampling(prior:list,conditions=None,nbModels:int=1000):
    ...
    if conditions is None:
        ...
        # Checking that the conditions are callable (lambda or def function)
        if not(callable(conditions)):
            raise Exception('conditions should be a callable. conditions is a {}'.format(type(conditions)))
        if nbModels > 1:
            # Sampling the models:
            achieved = False
            modelsOK = 0
...
def PropagateNoise(POSTBEL,NoiseLevel=None, DatasetIn=None, verbose:bool=False):
    ...
    dim = POSTBEL.CCA.n_components  # Number of dimensions for noise propagation (x_scores_ removed in sklearn>=1.1)
