...
c                                                                     c
c        COPYRIGHT 2024 R. B. Herrmann                                c
c                                                                     c
...
c
c        17 FEB 2025 - added more detail to the help
c-----
...
c-----
     ...
     1      dotest1,dostep,outfmt,dosdr,dohalf)
        WRITE(LER,*)'ntau              :',ntau
        WRITE(LER,*)'ipt               :',ipt 
        WRITE(LER,*)'alp               :',alp 
        WRITE(LER,*)'idva              :',idva
...
c-----
     ...
     2               mname,dostep,alpha,TP,TSV,TSH,dohalf)
            else
     ...
     2              mname,dostep,alpha,dosdr,TP,TSV,TSH,dohalf)
                 if(dostress)then
...
c       dohalf     -L .true. output only first N/2 points
c-----
        character*(*) rfile
        integer*4 ntau, ipt, idva
        real*4 alp
        logical  dozero, dotest1,dostep
        logical dostrain, dostress,dorotate,dogreen,dosdr,dohalf
...
c-----     
     ...
     1  '       The all use a z-positive down coordinate system.'
        write(LER,'(a/a/a/a/a)')
     ...
     1  '       with hpulse96. The coordinate system is such that ',
     1  '       Uz is positive upward. The radial and vertical ',
     1  '       partial derivatives with respect to the receiver',
     1  '       are also output, e.g., with ZDD_dr and ZDD_dz.',
...
c------
     ...
     1  '--------------------------------------------------------------'
        write(LER,*)
     1  'NOTE: The output units are related to the model specification.'
        write(LER,*)
     ...
     1  '--------------------------------------------------------------'
        write(LER,*)
     1  ' -HALF        (default false) output on first half of points'
...
c                   DDDDDddd_HHHhhh_ZZZzzz.cmp
                    ...
                    istel  = 1000.*depthr
                    idist  = 1000.*dist
                    write(ofile,12)idist,ievdp,istel,stcomp
