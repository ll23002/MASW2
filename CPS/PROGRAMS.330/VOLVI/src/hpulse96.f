...
c-----
 ...
 9000   continue
     ...
     1      rfile,ostr,dozero,dostep,dohalf)
c-----
c       parse command line arguments
...
c                         function.
c       dohalf  L   .false. output only the first half of the time series
c-----
        character*(*)  rfile
        integer*4 ntau, ipt, idva
        real*4 xmult, alp
...
c-----
   ...
   11   i = i + 1
            ...
            if(name(1:2).eq.'-l')then
                ...
                call usage('Help')
            else if(name(1:2).eq.'-h'.and.name(1:5).ne.'-HALF')then
                call usage('Help')
            endif
        go to 11
   13   continue
