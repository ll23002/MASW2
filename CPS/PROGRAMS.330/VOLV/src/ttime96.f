...
c                     added spherical earth to TI. Note the T* operator
c                     for the refraction is a HACK and may not be correct
c
c                     Note if we ask for S and there is a fluid layer, we compute
c                     S-Pfluid->S
...
c       11 JUNE 2009 - changed 'real mmax' to 'integer mmax' at line 1438
c       07 MAY 2025  - usage indicates that this works for TI models
c-----
        implicit none
        character mname*80
...
c-----
        ...
        parameter (LER=0)
        write(LER,*)ostr
        write(LER,*)'TI model travel times'
        write(LER,*)'Usage: ttime96 -GCARC gcarc -DIST dist '
       write(LER,*)'     ',
...
c-----
     ...
     3      frefp(NL), frefs(NL)
        ...
        real sumx, sumt, tt
        real time
        real vs, ts, vlmn, tint, vel, vl, vp, vlmx, vsh, vsv
