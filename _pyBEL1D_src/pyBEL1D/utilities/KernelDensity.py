...
def ParallelKernel(inputs):
    ...
    for j in bandPossible:
        ...
        # for k in range(nbTh):
        # [j*3*mt.cos(th[k]) + meanD, j*3*mt.sin(th[k]) + meanH]
        circleOK = np.column_stack(
            ((3*j*np.cos(th) + meanD), (3*j*np.sin(th) + meanH)))
    ...
    if XTrue is None:
        Xaxis = np.arange(
            (np.min(dataset[:, 0])-4*band), (np.max(dataset[:, 0])+4*band), band/2)
        Yaxis = np.arange(
            (np.min(dataset[:, 1])-4*band), (np.max(dataset[:, 1])+4*band), band/2)
        ...
        for x in Xaxis:
            ...
            for y in Yaxis:
                ...
                probaLarge = 4
                impacts = np.logical_and(np.logical_and(np.greater(dataset[:, 0], x-probaLarge*band), np.less(dataset[:, 0], x+probaLarge*band)), np.logical_and(
                    np.greater(dataset[:, 1], y-probaLarge*band), np.less(dataset[:, 1], y+probaLarge*band)))  # p.contains_points(dataset)
                if np.sum(impacts) > 0:
                    idxImpacts = np.where(impacts)
                    idxImpacts = idxImpacts[0]
                    # Test for vectorization:
                    z = np.divide(np.power(np.subtract(x, dataset[idxImpacts, 0]), 2), band**2) + np.divide(np.power(np.subtract(y, dataset[idxImpacts, 1]), 2), band**2) - np.divide(
                        np.multiply(np.multiply(np.subtract(x, dataset[idxImpacts, 0]), np.subtract(y, dataset[idxImpacts, 1])), 2), band*band)
        ...
        for y in Yaxis:
            # p = path.Path(circle(x,y,band))
            probaLarge = 4
            impacts = np.logical_and(np.logical_and(np.greater(dataset[:, 0], x-probaLarge*bandX), np.less(dataset[:, 0], x+probaLarge*bandX)), np.logical_and(
                np.greater(dataset[:, 1], y-probaLarge*band), np.less(dataset[:, 1], y+probaLarge*band)))  # p.contains_points(dataset)
            if np.sum(impacts) > 0:
                idxImpacts = np.where(impacts)
                idxImpacts = idxImpacts[0]
                # Test for vectorization:
                z = np.divide(np.power(np.subtract(x, dataset[idxImpacts, 0]), 2), bandX**2) + np.divide(np.power(np.subtract(y, dataset[idxImpacts, 1]), 2), bandY**2) - np.divide(
                    np.multiply(np.multiply(np.subtract(x, dataset[idxImpacts, 0]), np.subtract(y, dataset[idxImpacts, 1])), 2), bandX*bandY)
        ...
        KDE = np.divide(KDE, np.trapezoid(KDE, Yaxis))
...
class KDE:
    ...
    def KernelDensity(self, dim=None, XTrue=None, NoiseError=None, RemoveOutlier=False, Parallelization=[False, None], verbose: bool = False):
        ...
        if dim is None:
            ...
            raise Exception('Dimension outside of possibilities: max = {} (input = {})'.format(
                self.nb_dim, np.max(dim)))
        if XTrue is not None and len(XTrue) != self.nb_dim:
            raise Exception(
                'XTrue is not compatible with the dimensions given.')
            ...
            if NoiseError is None:
                for i in range(len(XTrue)):
                    ...
                    if XTrue[i] < np.quantile(dataset[:, 0], percentileLimit) or XTrue[i] > np.quantile(dataset[:, 0], 1-percentileLimit):
                        raise Exception(
                            'The dataset is outside of the distribution in the reduced space for dimension {}'.format(i+1))
            else:
                for i in range(len(XTrue)):
                    dataset = self.datasets[i]
                    if XTrue[i]+3*NoiseError[i] < np.quantile(dataset[:, 0], percentileLimit) or XTrue[i]-3*NoiseError[i] > np.quantile(dataset[:, 0], 1-percentileLimit):
                        raise Exception(
                            'The dataset is outside of the distribution in the reduced space for dimension {}. Even with noise taken into account!'.format(i+1))
        if Parallelization[0]:
            ...
            if Parallelization[1] is not None:
                ...
                # Create the parallel pool with at most the number of dimensions
                pool = pp.ProcessPool(np.min([len(dim), mp.cpu_count()]))
            inKernel = []
            for i in dim:
                if (XTrue is not None) and (NoiseError is not None):
                    inKernel.append(
                        [self.datasets[i], XTrue[i], NoiseError[i]])
            ...
            for i in dim:
                ...
                if RemoveOutlier:  # Modification for graph only:
                    ...
                    isoutlierX = np.greater(np.abs(
                        dataset[:, 0]), 3*c*np.median(np.abs(dataset[:, 0]-np.median(dataset[:, 0]))))
                    isoutlierY = np.greater(np.abs(
                        dataset[:, 1]), 3*c*np.median(np.abs(dataset[:, 1]-np.median(dataset[:, 1]))))
                    isoutlier = np.logical_or(isoutlierX, isoutlierY)
                    if any(isoutlier):
                        dataset = np.delete(dataset, np.where(isoutlier), 0)
                        LNew = dataset.shape
                        # Only keeping the length of the dataset
                        LNew = LNew[0]
                        if verbose:
                ...
                for j in bandPossible:
                    ...
                    if np.sum(inside) > np.max([L*0.05, 100]):
                        break
                band = j
                # Defing some lambda functions
                # 2D gaussian pdf:
                ...
                # circle = lambda X,Y,b: np.column_stack(((4*b*np.cos(th) + X), (4*b*np.sin(th) + Y)))
                if XTrue is None:
                    self.Xaxis[i] = np.arange(
                        (np.min(dataset[:, 0])-4*band), (np.max(dataset[:, 0])+4*band), band/2)
                    self.Yaxis[i] = np.arange(
                        (np.min(dataset[:, 1])-4*band), (np.max(dataset[:, 1])+4*band), band/2)
                    ...
                    for y in self.Yaxis[i]:
                        ...
                        if np.sum(impacts) > 0:
                            ...
                            z = np.divide(np.power(np.subtract(x, dataset[idxImpacts, 0]), 2), bandX**2) + np.divide(np.power(np.subtract(y, dataset[idxImpacts, 1]), 2), bandY**2) - np.divide(
                                np.multiply(np.multiply(np.subtract(x, dataset[idxImpacts, 0]), np.subtract(y, dataset[idxImpacts, 1])), 2), bandX*bandY)
                            pdf = np.multiply(
                                1/(2*np.pi*bandX*bandY), np.exp(-z))
                            KDE[x_idx, y_idx] += np.sum(pdf)
                            # End- test for vectorization
                            # for j in np.arange(len(idxImpacts)):
                            #     KDE[x_idx,y_idx] += pdf(x,y,dataset[idxImpacts[j],0],dataset[idxImpacts[j],1],band)
                    ...
                    KDE[KDE < thresholdKDE] = 0
                    self.KDE[i] = KDE  # np.divide(KDE,(np.sum(KDE)*band**2))
                    KDE = np.divide(KDE, np.trapezoid(KDE, self.Yaxis[i]))
                    CDF = np.cumsum(np.divide(KDE, np.sum(KDE)))
    ...
    def ShowKDE(self, dim=None, Xvals=None, Noise=None):
        ...
        if (Xvals is not None):
            ...
            if len(Xvals) < self.nb_dim:
                raise Exception(
                    'Xvals is not compatible with the current datasets')
        if (Noise is not None):
            Noise = np.squeeze(Noise)
            if len(Noise) < self.nb_dim:
                raise Exception(
                    'Noise is not compatible with the current datasets')
        for i in dim:
            ...
            if (self.KDE[i] is not None):
                ...
                # , vmin=np.max(self.KDE[i])/100, vmax=np.max(self.KDE[i]))
                im = ax.pcolormesh(
                    X_KDE, Y_KDE, np.transpose(self.KDE[i]))  # , cmap=cmap)
                ...
                for t in cb.ax.get_yticklabels():
                    ...
                    t.set_rotation(30)
                if (Xvals is not None):
                    ax.plot([Xvals[i], Xvals[i]],
                            np.asarray(ax.get_ylim()), 'r')
                    if (self.Dist[i] is not None):
                        # Add graph on the left with KDE distribution for Xvals
                        ax.yaxis.set_ticklabels([])
                        pyplot.subplots_adjust(left=0.4)
                        # pyplot.axes([0.15, 0.1, 0.10, 0.8])
                        ax_hist = divider.append_axes(
                            'left', size='15%', pad=0.1)
                        ax_hist.plot(np.squeeze(self.Dist[i][1]), np.squeeze(
                            self.Dist[i][0]), 'k')
                    ...
                    if Noise is not None:
                        ...
                        for k, mult in enumerate([-3, -2, -1, 1, 2, 3]):
                            ax.plot([Xvals[i]+mult*Noise[i], Xvals[i]+mult*Noise[i]],
                                    np.asarray(ax.get_ylim()), ':r', linewidth=linespec[k])
                else:
                    ax.set_ylabel('$m^c_{}$'.format(str(i+1)))
                pyplot.tight_layout()
    ...
    def GetDist(self, Xvals=[0], dim=None, Noise=None, verbose: bool = False):
        ...
        if dim is None:
            ...
            raise Exception(
                'Dimension outside of possibilities: max = {} (input = {})'.format(self.nb_dim, dim))
        ...
        for i in dim:
            if Noise is None:
                if Xvals[i] < np.min(self.Xaxis[i]) or Xvals[i] > np.max(self.Xaxis[i]):
                    # Clip instead of crash
                    import warnings
                    warnings.warn('Dataset projected outside KDE range in dim {}. Clipping to boundary.'.format(i))
                    Xvals[i] = np.clip(Xvals[i], np.min(self.Xaxis[i]), np.max(self.Xaxis[i]))
            else:
                if Xvals[i] < np.min(self.Xaxis[i])-3*Noise[i] or Xvals[i] > np.max(self.Xaxis[i])+3*Noise[i]:
                    # Clip instead of crash
                    import warnings
                    warnings.warn('Dataset projected outside KDE range in dim {}, even with noise. Clipping to boundary.'.format(i))
                    Xvals[i] = np.clip(Xvals[i], np.min(self.Xaxis[i]), np.max(self.Xaxis[i]))
        if Noise is None:
            for i in dim:
                # Find the first index that is greater than the value
                idx = np.searchsorted(self.Xaxis[i], Xvals[i], side='left')
                if Xvals[i] == self.Xaxis[i][idx]:
                    ...
                    KDE = self.KDE[i][idx-1, :] + ((self.KDE[i][idx, :]-self.KDE[i][idx-1, :])/(
                        self.Xaxis[i][idx]-self.Xaxis[i][idx-1]))*(Xvals[i]-self.Xaxis[i][idx-1])
                    KDE = np.divide(KDE, np.trapezoid(KDE, self.Yaxis[i]))
                    CDF = np.cumsum(np.divide(KDE, np.sum(KDE)))
                    Dist[i] = [[self.Yaxis[i]], [KDE], [CDF]]
        else:
            ...
            for i in dim:
                ...
                for j in range(samples):
                    # Find the first index that is greater than the value
                    idx = np.searchsorted(self.Xaxis[i], r[j], side='left')
                    if idx >= len(self.Xaxis[i]):
                        ...
                        KDE_tmp += self.KDE[i][idx-1, :] + ((self.KDE[i][idx, :]-self.KDE[i][idx-1, :])/(
                            self.Xaxis[i][idx]-self.Xaxis[i][idx-1]))*(r[j]-self.Xaxis[i][idx-1])
    ...
    def SampleKDE(self, nbSample: int = 1000, dim=None):
        ...
        Samples = np.asarray(distUnif.rvs(nbSample*len(dim))
                             ).reshape(nbSample, len(dim))
        for i in range(len(dim)):
            ...
            # pyplot.plot(x,y,'ro')
            # pyplot.show(block=False)
            InvCDFs = interp1d(x, y, bounds_error=False,
