      program hwhole96straij
c---------------------------------------------------------------------c
c                                                                     c
c      COMPUTER PROGRAMS IN SEISMOLOGY                                c
c      VOLUME VI                                                      c
c                                                                     c
c      PROGRAM: HWHOLE96STRAIN                                        c
c                                                                     c
c      COPYRIGHT 2024                                                 c
c      R. B. Herrmann                                                 c
...
c       08 DEC 2024 - modified to support the partial derivatives with
c            respect to z and r in order to compute strains
c-----
c-----
c     THIS PROGRAM COMPUTES THE SPECTRA OF THE WHOLESPACE GREEN's
...
c-----
     ...
     2  1,2,3,4,13,5,6,14,7,8,9,10,11,12,15/
        fwhich = -1.0
c-----
...
C                   endif
C               else
C                   if(i.eq.9 .or. i.eq. 10 .or. i.eq.16)then
C                       ksrc(i) = jsrc(lsrc(i))
C                   else
C                       ksrc(i) = 0
C                   endif
...
c-----
        ...
        subroutine setup(zs,rs)
        implicit real*8 (a-h,o-z)
        real*4 zs,rs
        double precision frac
        common/zrinit/fac(34),rsph,facr(34),facz(34),rr,zr

        rsph=dsqrt(dble(rs)**2 + dble(zs)**2)
        z = dble(zs)
        r = dble(rs)
        rr = r/rsph
        zr = z/rsph
...
c       Fz
c-----
        fac(1) = frac(r,z,rsph,0,1,3)
        fac(2) = frac(r,z,rsph,0,1,2)
c----
c       Fr
c-----
        fac(3) = frac(r,z,rsph,1,0,3)
        fac(4) = frac(r,z,rsph,1,0,2)
c-----
c       Frrr
c-----
        fac(5) = -9.*frac(r,z,rsph,1,0,5) + 15*frac(r,z,rsph,3,0,7)
        fac(6) = -9.*frac(r,z,rsph,1,0,4) + 15*frac(r,z,rsph,3,0,6)
        fac(7) = -3.*frac(r,z,rsph,1,0,3) + 6*frac(r,z,rsph,3,0,5)
        fac(8) = frac(r,z,rsph,3,0,4)

        facz(5) = 45*frac(r,z,rsph,1,1,7) - 105*frac(r,z,rsph,3,1,9)
        facr(5) = 45*frac(r,z,rsph,2,0,7) - 105*frac(r,z,rsph,4,0,9)
     1     -9.*frac(r,z,rsph,0,0,5) + 45.*frac(r,z,rsph,2,0,7)

        facz(6) = 36.*frac(r,z,rsph,1,1,6) -90.*frac(r,z,rsph,3,1,8)
        facr(6) = 36.*frac(r,z,rsph,2,0,6) -90.*frac(r,z,rsph,4,0,8)
     1        -9.*frac(r,z,rsph,0,0,4) + 45.*frac(r,z,rsph,2,0,6)

        facz(7) = 9.*frac(r,z,rsph,1,1,5) - 30.*frac(r,z,rsph,3,1,7)
        facr(7) = 9.*frac(r,z,rsph,2,0,5) - 30.*frac(r,z,rsph,4,0,7)
     1        -3.*frac(r,z,rsph,0,0,3) + 18.*frac(r,z,rsph,2,0,5)

        facz(8) = -4.*frac(r,z,rsph,3,1,6)
        facr(8) = -4.*frac(r,z,rsph,4,0,6) + 3*frac(r,z,rsph,2,0,4)
c-----
c       Fzzz, Fzzzz, Frzzz
c-----
        fac(9) = -9*frac(r,z,rsph,0,1,5) + 15*frac(r,z,rsph,0,3,7)
        fac(10) = -9*frac(r,z,rsph,0,1,4) + 15*frac(r,z,rsph,0,3,6)
        fac(11)= -3*frac(r,z,rsph,0,1,3) + 6*frac(r,z,rsph,0,3,5)
        fac(12) = frac(r,z,rsph,0,3,4)

        facz(9) = 45*frac(r,z,rsph,0,2,7) -105*frac(r,z,rsph,0,4,9)
     1      -9*frac(r,z,rsph,0,0,5) + 45*frac(r,z,rsph,0,2,7)
        facz(10) = 36*frac(r,z,rsph,0,2,6) -90*frac(r,z,rsph,0,4,8)
     1       -9*frac(r,z,rsph,0,0,4) + 45*frac(r,z,rsph,0,2,6)
        facz(11) = 9*frac(r,z,rsph,0,2,5) - 30*frac(r,z,rsph,0,4,7)
     1       -3*frac(r,z,rsph,0,0,3) + 18*frac(r,z,rsph,0,2,5)
        facz(12) = -4*frac(r,z,rsph,0,4,6) + 3*frac(r,z,rsph,0,2,4)

        facr(9) = 45*frac(r,z,rsph,1,1,7) - 105*frac(r,z,rsph,1,3,9)
        facr(10) = 36*frac(r,z,rsph,1,1,6) - 90*frac(r,z,rsph,1,3,8)
        facr(11) = 9*frac(r,z,rsph,1,1,5) - 30*frac(r,z,rsph,1,3,7)
        facr(12) = -4*frac(r,z,rsph,1,3,6)
c-----
c       frrz
c-----
        fac(13) = -3*frac(r,z,rsph,0,1,5) + 15*frac(r,z,rsph,2,1,7)
        fac(14) = -3*frac(r,z,rsph,0,1,4) + 15*frac(r,z,rsph,2,1,6)
        fac(15) = -frac(r,z,rsph,0,1,3) + 6*frac(r,z,rsph,2,1,5)
        fac(16) = frac(r,z,rsph,2,1,4)

        facz(13) = -3*frac(r,z,rsph,0,0,5) + 15*frac(r,z,rsph,2,0,7)
     1             + 15*frac(r,z,rsph,0,2,7) -105*frac(r,z,rsph,2,2,9)
        facz(14) = -3*frac(r,z,rsph,0,0,4) + 15*frac(r,z,rsph,2,0,6)
     1            +12*frac(r,z,rsph,0,2,6) - 90*frac(r,z,rsph,2,2,8)
        facz(15) = -frac(r,z,rsph,0,0,3) +3*frac(r,z,rsph,0,2,5)
     1            +6*frac(r,z,rsph,2,0,5) - 30*frac(r,z,rsph,2,2,7)
        facz(16) = frac(r,z,rsph,2,0,4) - 4*frac(r,z,rsph,2,2,6)

        facr(13) = 15*frac(r,z,rsph,1,1,7) - 105*frac(r,z,rsph,3,1,9)
     1           +30*frac(r,z,rsph,1,1,7)
        facr(14) = 12*frac(r,z,rsph,1,1,6) - 90*frac(r,z,rsph,3,1,8)
     1           +30*frac(r,z,rsph,1,1,6)
        facr(15) = 3*frac(r,z,rsph,1,1,5) + 12*frac(r,z,rsph,1,1,5)
     1           -30*frac(r,z,rsph,3,1,7)
        facr(16) = 2*frac(r,z,rsph,1,1,4) - 4*frac(r,z,rsph,3,1,6)
c-----
c       Frzz Frzzz Frrzz
c-----
        fac(17)= -3.*frac(r,z,rsph,1,0,5) + 15.*frac(r,z,rsph,1,2,7)
        fac(18)= -3.*frac(r,z,rsph,1,0,4) + 15.*frac(r,z,rsph,1,2,6)
        fac(19)= 6.*frac(r,z,rsph,1,2,5) - frac(r,z,rsph,1,0,3)
        fac(20)= frac(r,z,rsph,1,2,4)

        facz(17) = 15*frac(r,z,rsph,1,1,7) - 105*frac(r,z,rsph,1,3,9)
     1           +30*frac(r,z,rsph,1,1,7)
        facz(18) =  12*frac(r,z,rsph,1,1,6) + 30*frac(r,z,rsph,1,1,6)
     1           -90*frac(r,z,rsph,1,3,8)
        facz(19) = 3*frac(r,z,rsph,1,1,5) + 12*frac(r,z,rsph,1,1,5)
     1           - 30*frac(r,z,rsph,1,3,7)
        facz(20) = 2*frac(r,z,rsph,1,1,4) - 4*frac(r,z,rsph,1,3,6)

        facr(17) = -3*frac(r,z,rsph,0,0,5) + 15*frac(r,z,rsph,2,0,7)
     1           +15*frac(r,z,rsph,0,2,7) -105*frac(r,z,rsph,2,2,9)
        facr(18) = -3*frac(r,z,rsph,0,0,4) + 12*frac(r,z,rsph,2,0,6)
     1           +15*frac(r,z,rsph,0,2,6) - 90*frac(r,z,rsph,2,2,8)
        facr(19) = -frac(r,z,rsph,0,0,3) +3*frac(r,z,rsph,2,0,5)
     1           +6*frac(r,z,rsph,0,2,5) -30*frac(r,z,rsph,2,2,7)
        facr(20) = frac(r,z,rsph,0,2,4) - 4*frac(r,z,rsph,2,2,6)
c-----
c       Frz
c-----
        fac(21) = 3*frac(r,z,rsph,1,1,5)
        fac(22) = 3*frac(r,z,rsph,1,1,4)
        fac(23) = frac(r,z,rsph,1,1,3)
c-----
c       Fzz
c-----
        fac(24) = 3.*frac(r,z,rsph,0,2,5) - 1.*frac(r,z,rsph,0,0,3)
        fac(25) = 3.*frac(r,z,rsph,0,2,4) - 1.*frac(r,z,rsph,0,0,2)
        fac(26) = frac(r,z,rsph,0,2,3)
c-----
c       Frr
c-----
        fac(27) = 3.*frac(r,z,rsph,2,0,5) - 1.*frac(r,z,rsph,0,0,3)
        fac(28) = 3.*frac(r,z,rsph,2,0,4) - 1.*frac(r,z,rsph,0,0,2)
        fac(29) = frac(r,z,rsph,2,0,3)
c-----
c       Fror, Frorz, Frorr
c-----
        fac(30) = frac(r,z,rsph,0,0,3)
        fac(31) = frac(r,z,rsph,0,0,2)

        facz(30) = -3*frac(r,z,rsph,0,1,5)
        facz(31) = -2*frac(r,z,rsph,0,1,4)

        facr(30) = -3*frac(r,z,rsph,1,0,5)
        facr(31) = -2*frac(r,z,rsph,1,0,4)
...
c       Frzor
c-----
        fac(32) = 3.*frac(r,z,rsph,0,1,5)
        fac(33) = 3.*frac(r,z,rsph,0,1,4)
        fac(34) = frac(r,z,rsph,0,1,3)

        facz(32) = 3.*frac(r,z,rsph,0,0,5) -15*frac(r,z,rsph,0,2,7)
        facz(33) = 3.*frac(r,z,rsph,0,0,4) -12*frac(r,z,rsph,0,2,6)
        facz(34) = frac(r,z,rsph,0,0,3) -3*frac(r,z,rsph,0,2,5)

        facr(32) = -15*frac(r,z,rsph,1,1,7)
        facr(33) = -12*frac(r,z,rsph,1,1,6)
        facr(34) = -3*frac(r,z,rsph,1,1,5)
        return
        end
     ...
     1      frefp,frefs)
        ...
        complex*16 frrrr(2), frrrz(2), frrzz(2), frzzz(2), fzzzz(2)
        complex*16 frzorz(2), frzorr(2), frorz(2), frorr(2)
        complex*16 cresp(45)
        ...
        common/zrinit/fac(34),r1,facr(34),facz(34),rr,zr
        real*8 fac,r1,facr,facz,rr,zr
...
c-----
     ...
     2      fror(1), frzor(1) ,
     1  frrrr(1),frrrz(1), frrzz(1), frzzz(1), fzzzz(1),
     1  frzorz(1),frzorr(1),frorz(1),frorr(1))
c-----
...
c-----
     ...
     2      fror(2), frzor(2) ,
     2  frrrr(2),frrrz(2), frrzz(2), frzzz(2), fzzzz(2),
     1  frzorz(2),frzorr(2),frorz(2),frorr(2))
            if(b.gt.0.0)then
c-----
c       P contribution
c-----
...
c       RDDz
                    cresp(17) = cresp(17) - 
     1                  (3.0d+00*frzzz(1) 
     2                  + xka*xka*frz(1))/pi4ro
c       RDDr
                    cresp(32) = cresp(32) - 
     1                  (3.0d+00*frrzz(1) 
     2                  + xka*xka*frr(1))/pi4ro
c-----
c       ZDS
c-----
                    cresp(3) = cresp(3) + 
...
c       ZSSz
                    cresp(21) = cresp(21) - 
     1                  (2.0d+00*frrzz(1)+fzzzz(1)
     1                  +xka*xka*fzz(1)
     1                  )/pi4ro
c       ZSSr
                    cresp(36) = cresp(36) - 
     1                  (2.0d+00*frrrz(1)+frzzz(1)
     1                  +xka*xka*frz(1)
     1                  )/pi4ro
                    endif
c-----
...
c       RSSz
                    cresp(22) = cresp(22) -
     1                  (2.0d+00*frrrz(1)+frzzz(1)+xka*xka*frz(1)
     2                  )/pi4ro
c       RSSr
                    cresp(37) = cresp(37) -
     1                  (2.0d+00*frrrr(1)+frrzz(1)+xka*xka*frr(1)
     2                  )/pi4ro
c-----
c       TSS
c-----
...
c       TSSz
                    cresp(23) = cresp(23) +
     1                  ( -2.0d+00*frrrz(1)-2.0d+00*frzzz(1)
     1                  -2.0d+00*xka*xka*frz(1)
     3                  )/pi4ro
c       TSSr
                    cresp(38) = cresp(38) +
     1                  ( -2.0d+00*frrrr(1)-2.0d+00*frrzz(1)
     1                  -2.0d+00*xka*xka*frr(1)
     3                  )/pi4ro
c-----
c       ZEX
c-----
...
c       THFz
                    cresp(30) = cresp(30) - frorz(1)/pi4ro
c       THFr
                    cresp(45) = cresp(45) - frorr(1)/pi4ro
...
c       ZDDz
     ...
     1                  (
     1                  - 3.0d+00*fzzzz(2)
     2                  - 3.0d+00*xkb*xkb*fzz(2) )/pi4ro
c       ZDDr
                    cresp(31) = cresp(31) - 
     1                  (
     1                  - 3.0d+00*frzzz(2)
     2                  - 3.0d+00*xkb*xkb*frz(2) )/pi4ro
c-----
c       RDD
c-----
                    cresp(2) = cresp(2) - 
...
c       ZDSz
     ...
     1                   (
     1                  2.0d+00*frzzz(2) 
     2                  + xkb*xkb*frz(2))/pi4ro
c       ZDSr
                    cresp(33) = cresp(33) + 
     1                   (
     1                  2.0d+00*frrzz(2) 
     2                  + xkb*xkb*frr(2))/pi4ro
...
c       RDSz
                    cresp(19) = cresp(19) -
     1                   (2.0d+00*(-frrzz(2)) 
     1                  - xkb*xkb*fzz(2) )/pi4ro
c       RDSr
                    cresp(34) = cresp(34) -
     1                   (2.0d+00*(-frrrz(2)) 
     1                  - xkb*xkb*frz(2) )/pi4ro
c-----
c       TDS
c-----
                    cresp(5) = cresp(5) +
...
c       ZSSz
                    cresp(21) = cresp(21) - 
     1                  (
     1                  -2.0d+00*frrzz(2)-fzzzz(2)
     1                  -xkb*xkb*fzz(2))/pi4ro
c       ZSSr
                    cresp(36) = cresp(36) - 
     1                  (
     1                  -2.0d+00*frrrz(2)-frzzz(2)
     1                  -xkb*xkb*frz(2))/pi4ro
                    endif
...
c       RSSz
     ...
     1                  (
     1                  -2.0d+00*frrrz(2)-frzzz(2)
     2                  -2.0d+00*xkb*xkb*frz(2))/pi4ro
c       RSSr
                    cresp(37) = cresp(37) -
     1                  (
     1                  -2.0d+00*frrrr(2)-frrzz(2)
     2                  -2.0d+00*xkb*xkb*frr(2))/pi4ro
c-----
c       TSS
c-----
                    cresp(8) = cresp(8) +
...
c       TSSz
                    cresp(23) = cresp(23) +
     1                   ( 
     2                  + 2.0d+00*frrrz(2)+2.0d+00*frzzz(2)
     3                  +xkb*xkb*frz(2))/pi4ro
c       TSSr
                    cresp(38) = cresp(38) +
     1                   ( 
     2                  + 2.0d+00*frrrr(2)+2.0d+00*frrzz(2)
     3                  +xkb*xkb*frr(2))/pi4ro
c-----
c       ZEX
c-----
c-----
...
c       ZVFz
                    cresp(26) = cresp(26) + 
     1                  (- fzzz(2) - xkb*xkb*fz(2))/(pi4ro)
c       ZVFr
                    cresp(41) = cresp(41) + 
     1                  (- frzz(2) - xkb*xkb*fr(2))/(pi4ro)
c-----
c       RVF
c-----
                    cresp(12) = cresp(12) + 
...
c       RHFz
                    cresp(29) = cresp(29) +
     1                  ( - frrz(2) - xkb*xkb*fz(2))/pi4ro
c       RHFr
                    cresp(44) = cresp(44) +
     1                  ( - frrr(2) - xkb*xkb*fr(2))/pi4ro
c-----
c       THF
c-----
                    cresp(15) = cresp(15) -
...
c       THFz
                    cresp(30) = cresp(30) -
     1                  ( -frorz(2) - xkb*xkb*fz(2))/pi4ro
c       THFr
                    cresp(45) = cresp(45) -
     1                  ( -frorr(2) - xkb*xkb*fr(2))/pi4ro
                endif
c-----
c       SH contribution
c-----
...
C       cresp(13) = - cresp(13)
     ...
     1  frrrr,frrrz, frrzz, frzzz, fzzzz, frzorz,frzorr,
     1  frorz,frorr)
        common/zrinit/fac(34),r1,facr(34),facz(34),rr,zr
        real*8 fac,r1,facr,facz,rr,zr
        ...
        complex*16 frrrr, frrrz, frrzz, frzzz, fzzzz
        complex*16 fror, frzor, frzorz,frzorr,frorz,frorr

        complex*16 pfacr, pfacz
        ...
        pfacz = -kv * zr
            ...
            frorz= pfacz*fror
     1            -pfac * ( facz(30) + kv * facz(31))
            frorr= pfacr*fror
     1            -pfac * ( facr(30) + kv * facr(31))

            frz =  pfac*( fac(21) + kv*fac(22) + kv*kv*fac(23))
            frzor = pfac*( fac(32) + kv*fac(33) + kv*kv*fac(34))
            frzorz= pfacz*frzor
     1           +pfac*( facz(32) + kv*facz(33) + kv*kv*facz(34))
            frzorr= pfacr*frzor
     1           +pfac*( facr(32) + kv*facr(33) + kv*kv*facr(34))
     ...
     1          + kv*kv*kv*fac(8) )
            frrrr=  pfacr*frrr
     1          -pfac*( facr(5) + kv*facr(6) + kv*kv*facr(7)
     1          + kv*kv*kv*facr(8) )
            frrrz=  pfacz*frrr
     1          -pfac*( facz(5) + kv*facz(6) + kv*kv*facz(7)
     1          + kv*kv*kv*facz(8) )

            frrz = -pfac*( fac(13) + kv*fac(14) + kv*kv*fac(15)
     1          + kv*kv*kv*fac(16) )
            frrzz = pfacz*frrz
     1           -pfac*( facz(13) + kv*facz(14) + kv*kv*facz(15)
     1          + kv*kv*kv*facz(16) )
C           frrrz = pfacr*frrz
C    1           -pfac*( facr(13) + kv*facr(14) + kv*kv*facr(15)
C    1          + kv*kv*kv*facr(16) )

            frzz = -pfac*( fac(17) + kv*fac(18) + kv*kv*fac(19)
     1          + kv*kv*kv*fac(20) )
            frzzz= pfacz*frzz
     1           -pfac*( facz(17) + kv*facz(18) + kv*kv*facz(19)
     1          + kv*kv*kv*facz(20) )
            frrzz= pfacr*frzz
     1           -pfac*( facr(17) + kv*facr(18) + kv*kv*facr(19)
     1          + kv*kv*kv*facr(20) )

            fzzz = -pfac*( fac(9) + kv*fac(10) + kv*kv*fac(11)
     1          + kv*kv*kv*fac(12) )
            fzzzz= pfacz*fzzz 
     1            -pfac*( facz(9) + kv*facz(10) + kv*kv*facz(11)
     1          + kv*kv*kv*facz(12) )
            frzzz= pfacr*fzzz 
     1            -pfac*( facr(9) + kv*facr(10) + kv*kv*facr(11)
     1          + kv*kv*kv*facr(12) )
        return
        end
...
c-----
        ...
        double precision function frac(r,z,rsph,ir,iz,irsph)
        implicit none
        double precision r,z,rsph
        integer ir,iz, irsph
        integer i
c-----
c       form
c
c       r^ir z^iz / rsph^irsph
c-----
        frac = 1.0d+00
        if(ir.gt.0)then
           do i=1,ir
...
c-----
c       account for the last part of rsph
c-----
        frac = frac / rsph**(irsph -ir -iz)
        return
        end
