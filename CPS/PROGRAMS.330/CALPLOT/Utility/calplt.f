...
c       30 NOV 2024 - added arrow option to ARC
c                   - modified position of subscript in SUBSC
c-----
...
c-----
 1000   continue
     ...
     2              ' DIamond, TRiangle, HXagon' 
            ...
            if(verby)then
                write(6,*)'ARC:x0,y0,rad,ang1,ang2,arrow'
            endif
            read(5,*)x0,y0,rad,ang1,ang2,doarrow
            write(1,*)x0,y0,rad,ang1,ang2,doarrow
            call arc(rad,x0,y0,ang1,ang2,doarrow)
...
C     1             ipatx,ipaty,xlen,ylen)
 ...
 1000   continue
        ...
        subroutine arc(rad,x0,y0,ang1,ang2,doarrow)
        implicit none
        real rad, x0, y0, ang1, ang2
        real xx1, yy1, xx2, yy2
        logical doarrow

        real degrad, xx, yy
        integer i, i1, i2, inc, ipen
c-----
...
c-----
  ...
  100   continue
        call plot(xx,yy,3)
        if(doarrow)then
          write(6,*)xx1,yy1,xx2,yy2
          call arrow(xx1,yy1,xx2,yy2,.false.)
        endif
        return
        end
...
C        call plot(xxx, yyy, 3)
        ...
        if(n.gt.0)call symbol(x+nn*ht+0.1*ht,y-0.4*ht,0.7*ht,s2,0.0,n)
        if(n1.lt.0 .or. n2.lt.0)call gfont(infont)
        return
        end
...
c-----
     ...
     1  'ARRAY: enter quoted file name containing x,y pairs '
        write(LER,*)
     1  'ARC:x0,y0,rad,ang1,ang2,arrow'
        write(LER,*)
     1  '   ang1,ang2 - minimim, max angle wrt x-axis'
        write(LER,*)
     1  '   arrow .true. or .false. (default) at end'
        write(LER,*)
     1  '   if ang2 < ang1 - clockwise, else counterclockwise'
