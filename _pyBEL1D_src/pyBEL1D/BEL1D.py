...
# For parallelization (No issues with pickeling)
from pathos import multiprocessing as mp
from pathos import pools as pp              # For parallelization
# For building parallelizable functions
...
# Forward models:
try:
    # sNMR (from pyGIMLI: https://www.pygimli.org/)
    from pygimli.physics.sNMR import MRS, MRS1dBlockQTModelling
except:
    print('pyGiMLi not installed in environement')
try:
    # Dispersion Curves (from Github: https://github.com/hadrienmichel/pysurf96)
    from pysurf96 import surf96
except:
    print('surf96 not installed in environement')
...
def forwardPendulum(param, times):
    ...
    def f(t, in2): return np.asarray(
        [in2[1], omega_0**2 * np.sin(in2[0])]).transpose()
...
def forwardDCimage(freqsImage, velImage, model, nLayer):
    ...
    for i in range(periods/5):
        ...
        try:
            dispCurve[i*5:(i+1)*5] = surf96(thickness=np.append(model[0:nLayer-1], [0]), vp=model[2*nLayer-1:3*nLayer-1], vs=model[nLayer -
                                                                                                                                   1:2*nLayer-1], rho=model[3*nLayer-1:4*nLayer-1], periods=periodCurr, wave="rayleigh", mode=1, velocity="phase", flat_earth=True)
        except:
            fails += 1
    if fails == nbPeriods:
...
class MODELSET:
    ...
    def __init__(self, prior=None, cond=None, method=None, forwardFun=None, paramNames=None, nbLayer=None, logTransform=[False, False], thicknessesFixed=None):
        if (prior is None) or (method is None) or (forwardFun is None) or (paramNames is None):
            ...
            self.forwardFun = []
            self.paramNames = []
            # If None -> Model with parameters and no layers (not geophy?)
    ...
    def pendulum(cls, prior=None, timings=None):
        ...
        for i in range(nbParam):
            ListPrior[i] = stats.uniform(
                loc=prior[i*2], scale=prior[i*2+1]-prior[i*2])
            Mins[i] = prior[i*2]
            Maxs[i] = prior[i*2+1]
        ...
        method = "Pendulum"
        paramNames = {"NamesFU": NamesFullUnits, "NamesSU": NamesShortUnits, "NamesS": NamesShort, "NamesGlobal": NFull, "NamesGlobalS": [
            "Depth [m]", "W [/]", "T_2^* [sec]"], "DataUnits": "[V]", "DataName": "Amplitude [V]", "DataAxis": "Time/pulses [/]"}  # The representation is automated -> no time displayed since pulses are agregated

        forward = {"Fun": lambda model: forwardPendulum(
            model, timings), "Axis": timings}

        def cond(model): return np.logical_and(np.greater_equal(
            model[0]+model[1], H), (np.logical_and(np.greater_equal(model, Mins), np.less_equal(model, Maxs)))).all()
    ...
    def SNMR(cls, prior=None, Kernel=None, Timing=None):
        ...
        """
        if prior is None:
            prior = np.array([[2.5, 7.5, 0.035, 0.10, 0.005, 0.350], [
                             0, 0, 0.10, 0.30, 0.005, 0.350]])
        if Kernel is None:
        ...
        for j in range(nParam):
            for i in range(nLayer):
                if not ((i == nLayer-1) and (j == 0)):  # Not the half-space thickness
                    ListPrior[ident] = stats.uniform(
                        loc=prior[i, j*2], scale=prior[i, j*2+1]-prior[i, j*2])
                    Mins[ident] = prior[i, j*2]
                    Maxs[ident] = prior[i, j*2+1]
                    NamesFullUnits[ident] = NFull[j] + str(i+1) + Units[j]
                    NamesShortUnits[ident] = NShort[j] + \
                        str(i+1) + "}" + Units[j]
        ...
        def forwardFun(model): return ForwardSNMR(
            model, nLayer, KFile.K, KFile.z, Timing)
        forward = {"Fun": forwardFun, "Axis": Timing}
        def cond(model): return (np.logical_and(np.greater_equal(
            model, Mins), np.less_equal(model, Maxs))).all()
    ...
    def DC(cls, prior=None, Frequency=None):
        ...
        # from pysurf96 import surf96
        if prior is None:
            prior = np.array([[0.0025, 0.0075, 0.002, 0.1, 0.05, 0.5, 1.0, 3.0], [
                             0, 0, 0.1, 0.5, 0.3, 0.8, 1.0, 3.0]])
        ...
        paramNames = {"NamesFU": NamesFullUnits, "NamesSU": NamesShortUnits, "NamesS": NamesShort, "NamesGlobal": NFull, "NamesGlobalS": [
            "Depth\\ [km]", "Vs\\ [km/s]", "Vp\\ [km/s]", "\\rho\\ [T/m^3]"], "DataUnits": "[km/s]", "DataName": "Phase\\ velocity\\ [km/s]", "DataAxis": "Periods\\ [s]"}

        def forwardFun(model): return surf96(thickness=np.append(model[0:nLayer-1], [0]), vp=model[2*nLayer-1:3*nLayer-1], vs=model[nLayer -
                                                                                                                                    1:2*nLayer-1], rho=model[3*nLayer-1:4*nLayer-1], periods=Periods, wave="rayleigh", mode=1, velocity="phase", flat_earth=True)
        forward = {"Fun": forwardFun, "Axis": Periods}

        def PoissonRatio(model):
            vp = model[2*nLayer-1:3*nLayer-1]
            vs = model[nLayer-1:2*nLayer-1]
            ratio = 1/2 * (np.power(vp, 2) - 2*np.power(vs, 2)) / \
                (np.power(vp, 2)-np.power(vs, 2))
            return ratio
        RatioMin = [0.2]*nLayer
        RatioMax = [0.45]*nLayer
        def cond(model): return (np.logical_and(np.greater_equal(model, Mins), np.less_equal(model, Maxs))).all() and (
    ...
    def DCVs(cls, prior=None, Frequency=None, VpFixed=None, RhoFixed=None):
        ...
        RhoTypical = 2.0
        paramNames = {"NamesFU": NamesFullUnits, "NamesSU": NamesShortUnits, "NamesS": NamesShort, "NamesGlobal": NFull, "NamesGlobalS": [
            "Depth\\ [km]", "Vs\\ [km/s]"], "DataUnits": "[km/s]", "DataName": "Phase\\ velocity\\ [km/s]", "DataAxis": "Periods\\ [s]"}
        if (VpFixed is None) and (RhoFixed is None):
            def forwardFun(model): return surf96(thickness=np.append(model[0:nLayer-1], [0]), vp=np.sqrt((2*PoissonRatio-2)/(2*PoissonRatio-1) * np.power(
                model[nLayer-1:2*nLayer-1], 2)), vs=model[nLayer-1:2*nLayer-1], rho=np.ones((nLayer,))*RhoTypical, periods=Periods, wave="rayleigh", mode=1, velocity="phase", flat_earth=True)
        elif (VpFixed is not None) and (RhoFixed is not None):
            def forwardFun(model): return surf96(thickness=np.append(
                model[0:nLayer-1], [0]), vp=VpFixed, vs=model[nLayer-1:2*nLayer-1], rho=RhoFixed, periods=Periods, wave="rayleigh", mode=1, velocity="phase", flat_earth=True)
        elif VpFixed is not None:
            def forwardFun(model): return surf96(thickness=np.append(model[0:nLayer-1], [0]), vp=VpFixed, vs=model[nLayer-1:2*nLayer-1], rho=np.ones(
                (nLayer,))*RhoTypical, periods=Periods, wave="rayleigh", mode=1, velocity="phase", flat_earth=True)
        elif RhoFixed is not None:
            def forwardFun(model): return surf96(thickness=np.append(model[0:nLayer-1], [0]), vp=np.sqrt((2*PoissonRatio-2)/(2*PoissonRatio-1) * np.power(
                model[nLayer-1:2*nLayer-1], 2)), vs=model[nLayer-1:2*nLayer-1], rho=RhoFixed, periods=Periods, wave="rayleigh", mode=1, velocity="phase", flat_earth=True)
    ...
    def sNMR_logLayers(cls, prior=None, Kernel=None, Timing=None, logUniform=True, maxDepth: float = 150, minThick=0.05, maxThick=5, nbLayers: int = 25):
        ...
        """
        if prior is None:
            prior = np.repeat(np.atleast_2d(
                [0.025, 0.30, 0.005, 0.400]), nbLayers, axis=0)
        if Kernel is None:
            Kernel = "Data/sNMR/KernelTest.mrsk"
        if Timing is None:
            Timing = np.arange(0.005, 0.5, 0.001)
        ...
        for j in range(nParam):
            for i in range(nbLayers):
                if logUniform:
                    ListPrior[ident] = stats.loguniform(
                        a=prior[i, j*2], b=prior[i, j*2+1]-prior[i, j*2])
                else:
                    ListPrior[ident] = stats.uniform(
                        loc=prior[i, j*2], scale=prior[i, j*2+1]-prior[i, j*2])
                Mins[ident] = prior[i, j*2]
        ...
        def forwardFun(model): return ForwardSNMR(
            np.append(thickness[:-1], model), nbLayers, KFile.K, KFile.z, Timing)
        forward = {"Fun": forwardFun, "Axis": Timing}
        # np.logical_and(np.max(np.abs(np.diff(model[:nbLayers]))) < 0.05, np.max(np.abs(np.diff(model[nbLayers:]))) < 0.05))).all()
        def cond(model): return (np.logical_and(np.logical_and(
            np.greater_equal(model, Mins), np.less_equal(model, Maxs)), 1)).all()
    ...
    def DCVs_logLayers(cls, prior=None, Frequency=None, logUniform=True, maxDepth: float = 0.150, minThick=5e-4, maxThick=0.005, nbLayers: int = 50):
        ...
        linLayers = 0
        while notOK:
            depths = np.logspace(np.log10(minThick), np.log10(
                maxDepth-(linLayers*maxThick)), nbLayers-(linLayers+1))
            thickness = np.append([minThick], np.diff(depths))
            if thickness[-1] > maxThick:
                linLayers += 1
            else:
                notOK = False
        if linLayers > 0:
            depths = np.append(depths, np.cumsum(
                np.ones((linLayers,))*maxThick)+depths[-1])
            thickness = np.append([minThick], np.diff(depths))
        else:
            raise Exception(
                'Impossible to generate a thickness model with those parameters.')
        # prior = np.multiply(prior,np.matlib.repmat(np.array([1/1000, 1/1000, 1, 1, 1, 1, 1, 1]),nLayer,1))
        ...
        def forwardFun(model): return surf96(thickness=np.append(thickness, [0]), vp=np.sqrt((2*PoissonRatio-2)/(2*PoissonRatio-1) * np.power(
            model, 2)), vs=model, rho=np.ones((nbLayers,))*RhoTypical, periods=Periods, wave="rayleigh", mode=1, velocity="phase", flat_earth=True)
        forward = {"Fun": forwardFun, "Axis": Periods}
        def cond(model): return (np.logical_and(np.greater_equal(
            model, Mins), np.less_equal(model, Maxs))).all()
        return cls(prior=ListPrior, cond=cond, method=method, forwardFun=forward, paramNames=paramNames, nbLayer=nbLayers, logTransform=[False, False], thicknessesFixed=thickness)
...
class PREBEL:
    ...
    def run(self, Parallelization: list = [False, None], RemoveOutlier: bool = False, reduceModels: bool = False, verbose: bool = False, PriorSampled=None, PriorResponses=None):
        ...
        if PriorResponses is None:
            ...
            if PriorSampled is None:
                if self.nbModels is None:  # Normally, we should never enter this
                    ...
                    self.MODELS = Tools.Sampling(
                        self.PRIOR, self.CONDITIONS, self.nbModels)
            else:
            ...
            while True:
                try:
                    tmp = self.MODPARAM.forwardFun["Fun"](
                        self.MODELS[indexCurr, :])
                    break
                except:
                    indexCurr += 1
            ...
            if Parallelization[0]:
                ...
                functionParallel = partial(
                    ForwardParallelFun, function=self.MODPARAM.forwardFun["Fun"], nbVal=len(tmp))
                inputs = [self.MODELS[i, :] for i in range(self.nbModels)]
                if Parallelization[1] is not None:
                    pool = Parallelization[1]
                    terminatePool = False
                else:
                    # Create the pool for paralelization
                ...
                notComputed = [i for i in range(
                    self.nbModels) if self.FORWARD[i, 0] is None]
                self.MODELS = np.array(
                    np.delete(self.MODELS, notComputed, 0), dtype=np.float64)
                self.FORWARD = np.array(
                    np.delete(self.FORWARD, notComputed, 0), dtype=np.float64)
                # Get the number of models remaining
                newModelsNb = np.size(self.MODELS, axis=0)
                timeEnd = time.time()
                if verbose:
                    print('The Parallelized Forward Modelling took {} seconds.'.format(
                        timeEnd-timeBegin))
            else:
                notComputed = []
                for i in range(self.nbModels):
                    # print(i)
                    try:
                        self.FORWARD[i, :] = self.MODPARAM.forwardFun["Fun"](
                            self.MODELS[i, :])
                    except:
                ...
                timeEnd = time.time()
                if verbose:
                    print('The Unparallelized Forward Modelling took {} seconds.'.format(
                        timeEnd-timeBegin))
            if self.MODPARAM.method == "DC":
                ...
                VariabilityMax = np.max(
                    np.abs(self.FORWARD[:, 1:]-self.FORWARD[:, :-1]), axis=1)
                # https://github.com/PyCQA/pylint/issues/3744 pylint: disable=no-name-in-module
                from scipy.special import erfcinv
                c = -1/(mt.sqrt(2)*erfcinv(3/2))
                VariabilityMaxAuthorized = np.median(
                    VariabilityMax) + 3 * c*np.median(np.abs(VariabilityMax-np.median(VariabilityMax)))
                isOutlier = np.greater(
                    np.abs(VariabilityMax), VariabilityMaxAuthorized)
                self.MODELS = np.delete(self.MODELS, np.where(isOutlier), 0)
                self.FORWARD = np.delete(self.FORWARD, np.where(isOutlier), 0)
                # Get the number of models remaining
                newModelsNb = np.size(self.FORWARD, axis=0)
                pass
            if verbose:
                print('{} models remaining after forward modelling!'.format(
                    newModelsNb))
            self.nbModels = newModelsNb
        else:
            self.MODELS = PriorSampled
            self.FORWARD = PriorResponses
            self.nbModels = np.size(self.FORWARD, axis=0)
        ...
        if reduceModels:
            ...
            if n_CompPCA_Data < n_CompPCA_Mod:
                ...
                # Ensure at least the same number of dimensions
                pca_data = sklearn.decomposition.PCA(
                    n_components=n_CompPCA_Mod)
                if self.MODPARAM.logTransform[1]:
                    d_h = pca_data.fit_transform(np.log(self.FORWARD))
                else:
                    d_h = pca_data.fit_transform(self.FORWARD)
        ...
        cca_transform = sklearn.cross_decomposition.CCA(
            n_components=n_CompPCA_Mod)
        ...
        self.KDE.KernelDensity(RemoveOutlier=RemoveOutlier,
    ...
    def POSTBEL2PREBEL(cls, PREBEL, POSTBEL, Dataset=None, NoiseModel=None, nbMaxPrior: int = 1000000, Parallelization: list = [False, None], reduceModels: bool = False, verbose: bool = False):
        ...
        PrebelNew.FORWARD = np.append(
            PREBEL.FORWARD, POSTBEL.SAMPLESDATA, axis=0)
        # Get the number of sampled models
        ...
        if PrebelNew.nbModels > nbMaxPrior:
            idxKeep = np.random.choice(
                PrebelNew.nbModels, nbMaxPrior, replace=False)
        ...
        if reduceModels:
            ...
            if n_CompPCA_Data < n_CompPCA_Mod:
                if verbose:
                    print(
                        'The data space can be represented with fewer dimensions than the models!')
                # Ensure at least the same number of dimensions
                pca_data = sklearn.decomposition.PCA(
                    n_components=n_CompPCA_Mod)
                if PrebelNew.MODPARAM.logTransform[1]:
        ...
        if Dataset is not None:
            ...
            if NoiseModel is not None:
                Noise = np.sqrt(Tools.PropagateNoise(
                    PrebelNew, NoiseModel, DatasetIn=Dataset))
            else:
                Noise = None
        # 5) KDE:
        if verbose:
        ...
        if Dataset is None:
            ...
            PrebelNew.KDE.KernelDensity(XTrue=np.squeeze(
                d_obs_c), NoiseError=Noise, Parallelization=Parallelization, verbose=verbose)
        if verbose:
    ...
    def runMCMC(self, Dataset=None, NoiseModel=None, nbSamples: int = 100000, nbChains: int = 10, noData: bool = False, verbose: bool = False):
        ...
        if NoiseModel is None:
            raise Exception(
                'No noise model provided. Impossible to compute the likelihood')
        if len(NoiseModel) != len(Dataset):
            raise Exception(
                'NoiseModel should have the same size as the dataset')
        ...
        for j in range(nbChains):
            ...
            # Compute the initial covariance from the prior distribution
            Covariance = 0.01*np.cov(self.MODELS.T)
            passed = False
            passedVerbose = False
            while i < nbSamples:
                if i == 0:
                    # Sampling a random model from the prior distribution
                    sampleCurr = Tools.Sampling(
                        self.PRIOR, self.CONDITIONS, nbModels=1)
                else:
                    # Random change to the sampled model (according to the covariance):
                    sampleCurr = sampleLast + \
                        np.random.multivariate_normal(
                            np.zeros((nbParam,)), Covariance)
                # Computing the likelihood from a data misfit:
                if self.MODPARAM.cond(sampleCurr):
                    try:
                        SynData = self.MODPARAM.forwardFun['Fun'](
                            sampleCurr[0])
                        DataDiff = Dataset - SynData
                        FieldError = NoiseModel
                        A = np.divide(1, np.sqrt(
                            2*np.pi*np.power(FieldError, 2)))
                        B = np.exp(-1/2 *
                                   np.power(np.divide(DataDiff, FieldError), 2))
                ...
                if np.mod(i, int(nbSamples/100)) == 0 and not (passedVerbose):
                    if verbose:
                        print('{} models sampled (out of {}) in chain {}.'.format(
                            i, nbSamples, j))
                    passedVerbose = True
                ...
                if (i < nbSamples/2) and (np.mod(i, 50) == 0) and not (passed):
                    ...
                    if AcceptanceRatio < 0.80:  # 0.75
                        ...
                        if verbose:
                            print(
                                'Acceptance ratio too high, increasing covariance.')
    ...
    def ShowPreModels(self, TrueModel=None):
        ...
        if nbLayer is not None:  # If the model can be displayed as layers
            # Number of parameters minus the thickness
            nbParamUnique = int(np.ceil(nbParam/nbLayer))-1
            ...
            if nbParamUnique > 1:
                ...
                axes = fig.subplots(1, nbParamUnique)
                for j in range(nbParamUnique):
                    for i in sortIndex:
                        axes[j].step(np.append(Param[j+1][i, :], Param[j+1][i, -1]), np.append(
                            np.append(0, Param[0][i, :]), maxDepth), where='pre', color='gray')
                    if TrueModel is not None:
                        axes[j].step(np.append(TrueMod[j+1][:], TrueMod[j+1][-1]), np.append(
                            np.append(0, TrueMod[0][:]), maxDepth), where='pre', color='k')
                    axes[j].invert_yaxis()
                    axes[j].set_ylim(bottom=maxDepth, top=0.0)
                    axes[j].set_xlabel(r'${}$'.format(
                        self.MODPARAM.paramNames["NamesGlobalS"][j+1]), fontsize=14)
                    axes[j].set_ylabel(r'${}$'.format(
                        self.MODPARAM.paramNames["NamesGlobalS"][0]), fontsize=14)
            else:
                j = 0
                # One graph per parameter
                axes = fig.subplots(1, nbParamUnique)
                for i in sortIndex:
                    axes.step(np.append(Param[j+1][i, :], Param[j+1][i, -1]), np.append(
                        np.append(0, Param[0][i, :]), maxDepth), where='pre', color='gray')
                if TrueModel is not None:
                    axes.step(np.append(TrueMod[j+1][:], TrueMod[j+1][-1]), np.append(
                        np.append(0, TrueMod[0][:]), maxDepth), where='pre', color='k')
                axes.invert_yaxis()
                axes.set_ylim(bottom=maxDepth, top=0.0)
                axes.set_xlabel(r'${}$'.format(
                    self.MODPARAM.paramNames["NamesGlobalS"][j+1]), fontsize=14)
    ...
    def ShowPriorDataset(self):
        ...
        for j in sortIndex:
            ax.plot(self.MODPARAM.forwardFun["Axis"], np.squeeze(
                self.FORWARD[j, :]), color='gray')
            ax.set_xlabel(r'${}$'.format(
                self.MODPARAM.paramNames["DataAxis"]), fontsize=14)
            ax.set_ylabel(r'${}$'.format(
                self.MODPARAM.paramNames["DataName"]), fontsize=14)
        pyplot.show(block=False)
...
class POSTBEL:
    ...
    def run(self, Dataset, nbSamples: int = 1000, Graphs: bool = False, NoiseModel: list = None, verbose: bool = False):
        ...
        if self.MODPARAM.cond is None:
            ...
            samples_PCA *= self.CCA._y_std
            samples_PCA += self.CCA._y_mean
            ...
            if type(self.MODPARAM.logTransform[0]) is list:
                if len(self.MODPARAM.logTransform[0]) != samples_Init.shape[1]:
                    raise Exception(
                        'The length of the log-transform is not the same as the length of the prior.')
                for count, logT in enumerate(self.MODPARAM.logTransform[0]):
                    if logT:
            ...
            nbParam = self.MODELS.shape[1]  # len(self.MODPARAM.prior)
            Samples = np.zeros((nbSamples, nbParam))
            achieved = False
            modelsOK = 0
            # At max, we could be at 0.1 sample every loop.
            nbTestsMax = nbSamples*10
            while not (achieved):
                ...
                if self.PCA['Model'] is None:
                    ...
                    Samples[modelsOK:, :] = self.PCA['Model'].inverse_transform(
                        samples_PCA)
                if type(self.MODPARAM.logTransform[0]) is list:
                ...
                if nbTestsMax < 0:
                    raise Exception(
                        'Impossible to sample models in the current posterior under reasonable timings!')
            if verbose:
                print(
                    '{} models sampled from the posterior model space!'.format(nbSamples))
    ...
    def runMCMC(self, NoiseModel=None, nbSamples: int = 20000, nbChains=10, verbose: bool = False):
        ...
        nbParam = len(self.MODPARAM.prior)
        accepted = np.zeros((nbChains, nbSamples, nbParam))
        acceptedData = np.zeros(
            (nbChains, nbSamples, len(self.DATA['True'][0, :])))
        for j in range(nbChains):
            if verbose:
                print('Running chain {} out of {}. . .'.format(j, nbChains))
            rejectedNb = 0
            i = 0
            LikelihoodLast = 1e-50  # First sample most likely accepted
            # Compute the initial covariance from the posterior distribution
            Covariance = 0.01*np.cov(self.SAMPLES.T)
            passed = False
            passedVerbose = False
            while i < nbSamples:
                if i == 0:
                    ...
                    # Back transform models to original space:
                    samples_PCA = np.matmul(
                        samples_CCA, self.CCA.y_loadings_.T)
                    samples_PCA *= self.CCA.y_std_
                    samples_PCA += self.CCA.y_mean_
                    if self.PCA['Model'] is None:
                        sampleCurr = samples_PCA
                    else:
                        sampleCurr = self.PCA['Model'].inverse_transform(
                            samples_PCA)
                else:
                    # Random change to the sampled model:
                    # np.random.uniform(0,0.01)*np.random.randn()*sampleAdd
                    sampleCurr = sampleLast + \
                        np.random.multivariate_normal(
                            np.zeros((len(self.MODPARAM.prior),)), Covariance)
                # Computing the likelihood from a data misfit:
    ...
    def DataPost(self, Parallelization=[False, None], OtherModels=None, verbose: bool = False):
        ...
        if Parallelization[0]:
            ...
            notComputed = [i for i in range(
                nbSamples) if SAMPLESDATA[i, 0] is None]
            SAMPLES = np.array(
                np.delete(SAMPLES, notComputed, 0), dtype=np.float64)
            SAMPLESDATA = np.array(
                np.delete(SAMPLESDATA, notComputed, 0), dtype=np.float64)
            # Get the number of models remaining
            newSamplesNb = np.size(SAMPLES, axis=0)
            if terminatePool:
                pool.terminate()
        else:
            notComputed = []
            for i in range(nbSamples):
                # print(i)
                try:
                    SAMPLESDATA[i, :] = self.MODPARAM.forwardFun["Fun"](
                        SAMPLES[i, :])
                except:
        ...
        if self.MODPARAM.method == "DC":
            ...
            # are luckily easy to identify. Their maximum variabilty is way larger than the other models.
            VariabilityMax = np.max(
                np.abs(SAMPLESDATA[:, 1:]-SAMPLESDATA[:, :-1]), axis=1)
            # https://github.com/PyCQA/pylint/issues/3744 pylint: disable=no-name-in-module
            from scipy.special import erfcinv
            c = -1/(mt.sqrt(2)*erfcinv(3/2))
            VariabilityMaxAuthorized = np.median(
    ...
    def runRejection(self, NoiseModel=None, Parallelization=[False, None], verbose: bool = False):
        ...
        if verbose:
            print(
                f'Rejection sampling ({len(ModelsAccepted)} out of {len(LikelihoodOrder)} kept) on POSTBEL executed in {time.time()-timeIn} seconds.')
            ...
            if i != nbParam-1:
                if prior:
                    if priorOther is None:
                        ax.hist(self.MODELS[:, i], density=True,
                                alpha=0.5, label='_Prior')
                    else:
                        ax.hist(priorOther[:, i], density=True,
                                alpha=0.5, label='_Prior')
                ax.hist(self.SAMPLES[:, i], density=True,
                        alpha=0.5, label='_Posterior')
                if TrueModel is not None:
                    ax.plot([TrueModel[i], TrueModel[i]], np.asarray(
                        ax.get_ylim()), 'k', label='_True')
            else:
                if priorOther is None:
                    ax.hist(self.MODELS[:, i], density=True,
    ...
    def ShowPostCorr(self, TrueModel=None, OtherMethod=None, OtherInFront=False, alpha=[1, 1], OtherModels=None):
        ...
        for i in range(nbParam):
            for j in range(nbParam):
                if i == j:  # Diagonal
                    if i != nbParam-1:
                        axs[i, j].get_shared_x_axes().join(
                            axs[i, j], axs[-1, j])  # Set the xaxis limit
                    if OtherInFront:
                        if OtherModels is not None:
                            # Plot the histogram for the given variable
                            axs[i, j].hist(OtherModels[:, j],
                                           color='b', density=True)
                        else:
                            # Plot the histogram for the given variable
                            axs[i, j].hist(self.SAMPLES[:, j],
                                           color='b', density=True)
                        if OtherMethod is not None:
                            axs[i, j].hist(OtherMethod[:, j],
                                           color='y', density=True)
                    else:
                        if OtherMethod is not None:
                            axs[i, j].hist(OtherMethod[:, j],
                                           color='y', density=True)
                        if OtherModels is not None:
                            ...
                            axs[i, j].hist(self.SAMPLES[:, j],
                                           color='b', density=True)
                    if TrueModel is not None:
                        axs[i, j].plot([TrueModel[i], TrueModel[i]],
                                       np.asarray(axs[i, j].get_ylim()), 'r')
                    if nbParam > 8:
                        axs[i, j].set_xticks([])
                        axs[i, j].set_yticks([])
                    ...
                    if j != nbParam-1:
                        if i != nbParam-1:
                            axs[i, j].get_shared_y_axes().join(
                                axs[i, j], axs[i, -1])  # Set the yaxis limit
                        else:
                            axs[i, j].get_shared_y_axes().join(
                                axs[i, j], axs[i, -2])  # Set the yaxis limit
                    if OtherModels is not None:
                        axs[i, j].plot(OtherModels[:, j], OtherModels[:, i],
                                       '.b', alpha=alpha[0], markeredgecolor='none')
                    else:
                        axs[i, j].plot(self.SAMPLES[:, j], self.SAMPLES[:, i],
                                       '.b', alpha=alpha[0], markeredgecolor='none')
                    if TrueModel is not None:
                    ...
                    if j != nbParam-1:
                        if i != 0:
                            ...
                            axs[i, j].get_shared_y_axes().join(
                                axs[i, j], axs[i, -2])  # Set the yaxis limit
                    axs[i, j].plot(OtherMethod[:, j], OtherMethod[:, i],
                                   '.y', alpha=alpha[1], markeredgecolor='none')
                    if TrueModel is not None:
                        axs[i, j].plot(TrueModel[j], TrueModel[i], 'or')
                    if nbParam > 8:
                        axs[i, j].set_xticks([])
                ...
                if j == 0:  # First column of the graph
                    if ((i == 0) and (j == 0)) or not (i == j):
                        axs[i, j].set_ylabel(r'${}$'.format(
                            self.MODPARAM.paramNames[namesVar][i]))
                if i == nbParam-1:  # Last line of the graph
                    axs[i, j].set_xlabel(r'${}$'.format(
                        self.MODPARAM.paramNames[namesVar][j]))
                if j == nbParam-1:
                    if not (i == j):
    ...
    def ShowPostModels(self, TrueModel=None, RMSE: bool = False, Best: int = None, Parallelization=[False, None], NoiseModel=None, OtherModels=None, OtherData=None, OtherRMSE=False):
        ...
        if RMSE:
            ...
            if OtherData is not None:
                RMS = np.sqrt(np.square(np.divide(np.subtract(
                    TrueData, OtherData), NoiseEstimation)).mean(axis=-1))
                if OtherRMSE:
                    RMS_scale = RMS
                else:
                    RMS_scale = np.sqrt(np.square(np.divide(np.subtract(
                        TrueData, self.SAMPLESDATA), NoiseEstimation)).mean(axis=-1))
            else:
                RMS = np.sqrt(np.square(np.divide(np.subtract(
                    TrueData, self.SAMPLESDATA), NoiseEstimation)).mean(axis=-1))
                RMS_scale = RMS
            quantiles = np.divide([stats.percentileofscore(
                RMS_scale, a, 'strict') for a in RMS], 100)
        ...
        if nbLayer is not None:  # If the model can be displayed as layers
            ...
            if self.MODPARAM.thicknessesFixed is None:
                ...
                for i in range(nbParamUnique):
                    Param.append(
                        ModelsPlot[:, (i+1)*nbLayer-1:(i+2)*nbLayer-1])
            else:
                Param.append(np.repeat(np.atleast_2d(
                    np.cumsum(self.MODPARAM.thicknessesFixed[:-1])), ModelsPlot.shape[0], axis=0))
                for i in range(nbParamUnique):
            ...
            if TrueModel is not None:
                ...
                if self.MODPARAM.thicknessesFixed is None:
                    ...
                    TrueMod.append(
                        np.cumsum(self.MODPARAM.thicknessesFixed[:-1]))
                for i in range(nbParamUnique):
                    if self.MODPARAM.thicknessesFixed is None:
                        TrueMod.append(
                            TrueModel[(i+1)*nbLayer-1:(i+2)*nbLayer-1])
                    else:
            ...
            if RMSE:
                ...
                # One graph per parameter
                gridSpec = fig.add_gridspec(9, nbParamUnique)
                axes = []
                ...
                for j in range(nbParamUnique):
                    for i in sortIndex:
                        axes[j].step(np.append(Param[j+1][i, :], Param[j+1][i, -1]), np.append(
                            np.append(0, Param[0][i, :]), maxDepth), where='pre', color=colormap(quantiles[i]))
                    if TrueModel is not None:
                        axes[j].step(np.append(TrueMod[j+1][:], TrueMod[j+1][-1]), np.append(
                            np.append(0, TrueMod[0][:]), maxDepth), where='pre', color='gray')
                    axes[j].invert_yaxis()
                    axes[j].set_ylim(bottom=maxDepth, top=0.0)
                    axes[j].set_xlabel(r'${}$'.format(
                        self.MODPARAM.paramNames["NamesGlobalS"][j+1]), fontsize=14)
        ...
        if RMSE:
            ...
            nb_inter = 1000
            color_for_scale = colormap(
                np.linspace(0, 1, nb_inter, endpoint=True))
            cmap_scale = colors.ListedColormap(color_for_scale)
            scale = [stats.scoreatpercentile(RMS_scale, a, limit=(np.min(RMS_scale), np.max(
                RMS_scale)), interpolation_method='lower') for a in np.linspace(0, 100, nb_inter, endpoint=True)]
            norm = colors.BoundaryNorm(scale, len(color_for_scale))
            data = np.atleast_2d(np.linspace(
                np.min(RMS_scale), np.max(RMS_scale), nb_inter, endpoint=True))
            ax_colorbar.imshow(data, aspect='auto', cmap=cmap_scale, norm=norm)
            if NoiseModel is None:
                ax_colorbar.set_xlabel('Root Mean Square Error {}'.format(
                    self.MODPARAM.paramNames["DataUnits"]), fontsize=12)
            else:
                ax_colorbar.set_xlabel(
                    'Noise Weighted Root Mean Square Error [/]', fontsize=12)
            ax_colorbar.yaxis.set_visible(False)
            nbTicks = 5
            ax_colorbar.set_xticks(ticks=np.linspace(
                0, nb_inter, nbTicks, endpoint=True))
            ax_colorbar.set_xticklabels(labels=round_to_n([stats.scoreatpercentile(RMS_scale, a, limit=(np.min(RMS_scale), np.max(
                RMS_scale)), interpolation_method='lower') for a in np.linspace(0, 100, nbTicks, endpoint=True)], n=2), rotation=30, ha='right')
    ...
    def ShowDataset(self, RMSE: bool = False, Prior: bool = False, Best: int = None, Parallelization=[False, None], OtherData=None):
        ...
        if RMSE:
            TrueData = self.DATA['True']
            if OtherData is not None:
                RMS = np.sqrt(np.square(np.subtract(
                    TrueData, OtherData)).mean(axis=-1))
                RMS_scale = np.sqrt(np.square(np.subtract(
                    TrueData, self.SAMPLESDATA)).mean(axis=-1))
            else:
                RMS = np.sqrt(np.square(np.subtract(
                    TrueData, self.SAMPLESDATA)).mean(axis=-1))
                RMS_scale = RMS
        ...
        if Prior:
            for j in range(self.nbModels):
                ax.plot(self.MODPARAM.forwardFun["Axis"], np.squeeze(
                    self.FORWARD[j, :len(self.MODPARAM.forwardFun["Axis"])]), color='gray')
        if OtherData is not None:
            PlotData = OtherData
        else:
        ...
        if RMSE:
            colormap = matplotlib.cm.get_cmap('viridis')
            for j in sortIndex:
                ax.plot(self.MODPARAM.forwardFun["Axis"], np.squeeze(PlotData[j, :len(
                    self.MODPARAM.forwardFun["Axis"])]), color=colormap(quantiles[j]))
            ax.set_xlabel(r'${}$'.format(
                self.MODPARAM.paramNames["DataAxis"]), fontsize=14)
            ax.set_ylabel(r'${}$'.format(
                self.MODPARAM.paramNames["DataName"]), fontsize=14)
            ...
            for j in sortIndex:
                ax.plot(self.MODPARAM.forwardFun["Axis"], np.squeeze(
                    PlotData[j, :len(self.MODPARAM.forwardFun["Axis"])]), color='gray')
            ax.set_xlabel(r'${}$'.format(
                self.MODPARAM.paramNames["DataAxis"]), fontsize=14)
            ax.set_ylabel(r'${}$'.format(
                self.MODPARAM.paramNames["DataName"]), fontsize=14)
        if RMSE:
            ...
            # fig.add_axes([0.10, 0.15, 0.80, 0.05])
            ...
            ax_colorbar.set_xticks(ticks=np.linspace(
                0, nb_inter, nbTicks, endpoint=True))
            ax_colorbar.set_xticklabels(labels=round_to_n([stats.scoreatpercentile(RMS_scale, a, limit=(np.min(RMS_scale), np.max(
                RMS_scale)), interpolation_method='lower') for a in np.linspace(0, 100, nbTicks, endpoint=True)], n=5), rotation=15, ha='center')
        pyplot.tight_layout()
        pyplot.show(block=False)
...
def SaveSamples(CurrentPostbel: POSTBEL, Data=False, Filename='Models_Sampled'):
    ...
    if Data:
        ...
        np.savetxt(Filename+'.datas',
                   CurrentPostbel.SAMPLESDATA, delimiter='\t')
...
def IPR(MODEL: MODELSET, Dataset=None, NoiseEstimate=None, Parallelization: list = [False, None],
    ...
    if MODEL.method == 'DC':
        # To make sure to have enough models after failed modllings.
        nbModelsBase = int(nbModelsBase*1.2)
    if Dataset is None:
    ...
    if PriorSampled is not None:
        Prebel.run(Parallelization=Parallelization, RemoveOutlier=True,
                   reduceModels=reduceModels, verbose=verbose, PriorSampled=PriorSampled)
    else:
        Prebel.run(Parallelization=Parallelization,
                   reduceModels=reduceModels, verbose=verbose)
    PrebelInit = Prebel
    ...
    for it in range(nbIterMax):
        ...
        if Mixing is not None:
            ...
            # ModPrebel/(1-Rejection)) # We need to sample at least this number of models to be able to add to the prior with mixing satisfied
            nbPostAdd = int((MixingUsed*nbModelsBase)/(1-Rejection))
            nbSamples = max([int(nbModelsSample/(1-Rejection)), nbPostAdd])
            if Prebel.MODPARAM.method is 'DC':
                # Additional 20% to reach the number with failed models
                nbSamples = int(nbSamples*1.2)
        else:
        ...
        Postbel.run(Dataset=Dataset, nbSamples=nbSamples,
                    NoiseModel=NoiseEstimate, verbose=verbose)
        # End of the iteration - begining of the preparation for the next iteration (if needed):
        end = time.time()
        if Graphs:
            if it == 0:
                NoiseToLastPrebel = Tools.PropagateNoise(
                    Postbel, NoiseLevel=NoiseEstimate, verbose=verbose)
                Postbel.KDE.ShowKDE(Xvals=Postbel.CCA.transform(Postbel.PCA['Data'].transform(
                    np.reshape(Dataset, (1, -1)))), Noise=NoiseToLastPrebel)
        ...
        # Power law defined from the different tests
        threshold = 1.87*nbConvergeSamp**(-0.50)
        diverge, distance = Tools.ConvergeTest(
            SamplesA=ModelLastIter, SamplesB=Postbel.SAMPLES, tol=threshold)
        if verbose:
            print('KS distance at iter {}: {} (threshold at {}).'.format(
        ...
        if not (diverge):
            ...
            if Graphs:
                NoiseToLastPrebel = Tools.PropagateNoise(
                    Postbel, NoiseLevel=NoiseEstimate, verbose=verbose)
                Postbel.KDE.KernelDensity(
                    NoiseError=NoiseToLastPrebel, RemoveOutlier=True, verbose=verbose)
                Postbel.KDE.ShowKDE(Xvals=Postbel.CCA.transform(Postbel.PCA['Data'].transform(
                    np.reshape(Dataset, (1, -1)))), Noise=NoiseToLastPrebel)
            break
        ...
        if Rejection > 0:
            RMSE = np.sqrt(np.square(np.subtract(
                Dataset, PostbelAdd.SAMPLESDATA)).mean(axis=-1))
            # We reject the x% worst fit
            RMSE_max = np.quantile(RMSE, 1-Rejection)
            idxDelete = np.greater_equal(RMSE, RMSE_max)
            PostbelAdd.SAMPLES = np.delete(
                PostbelAdd.SAMPLES, np.where(idxDelete), 0)
            PostbelAdd.SAMPLESDATA = np.delete(
                PostbelAdd.SAMPLESDATA, np.where(idxDelete), 0)
            PostbelAdd.nbSamples = np.size(PostbelAdd.SAMPLES, axis=0)
            # We update the number of samples needed (for mixing)
            nbPostAdd = int(nbPostAdd*(1-Rejection))
        if Mixing is not None:
            ...
            if (nbPostAdd < nbModelsSample) and (PostbelAdd.nbSamples > nbPostAdd):
                ...
                idxKeep = random.sample(
                    range(PostbelAdd.nbSamples), nbModelsSample)
        ...
        Prebel = PREBEL.POSTBEL2PREBEL(PREBEL=Prebel, POSTBEL=PostbelAdd, Dataset=Dataset, NoiseModel=NoiseEstimate,
                                       Parallelization=Parallelization, reduceModels=reduceModels, verbose=verbose)
    if Graphs:
        # plot the different graphs for the analysis of the results:
        pyplot.figure()
        pyplot.plot(range(len(statsReturn)), [
                    statsReturn[i].timing for i in range(len(statsReturn))])
        ax = pyplot.gca()
        ...
        for j in range(nbParam):
            ...
            ax.plot(range(len(statsReturn)), [
                    statsReturn[i].means[j] for i in range(len(statsReturn))], 'b-')
            ax.plot(range(len(statsReturn)), [
                    statsReturn[i].means[j]+statsReturn[i].stds[j] for i in range(len(statsReturn))], 'b--')
            ax.plot(range(len(statsReturn)), [
                    statsReturn[i].means[j]-statsReturn[i].stds[j] for i in range(len(statsReturn))], 'b--')
            if TrueModel is not None:
                ax.plot([0, len(statsReturn)-1],
                        [TrueModel[j], TrueModel[j]], 'r')
            ax.set_xlim(0, len(statsReturn)-1)
            ax.set_title(r'${}$'.format(
                Prebel.MODPARAM.paramNames["NamesFU"][j]))
